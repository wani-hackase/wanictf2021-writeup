...
title: "EmoEmotet"
level: 4
flag: "FLAG{w0w_7h3_3mb3dd3d_vb4_1n_w0rd_4u70m471c4lly_3x3cu73d_7h3_p0w3r5h3ll_5cr1p7}"
writer: "hi120ki"
zip: false
---
...
まず[oletools](https://github.com/decalage2/oletools)でdocファイルの中にあるマクロを取り出します
...
```
olevba 0.60 on Python 3.8.11 - http://decalage.info/python/oletools
===============================================================================
FILE: emoemotet.doc
Type: OLE
WARNING  Chunk size is larger than remaining compressed data
-------------------------------------------------------------------------------
VBA MACRO emo
...
難読化されたVBAマクロが取り出せたのでこれを復元します(macro.py)
...
Base64でエンコードされたpowershellスクリプトを実行しているのでこれをデコードします(pwshdec.ps1)
...
このpowershellスクリプトの冒頭の`iex`部分を取り除いて実行すると元のスクリプトを取得できます

> `iex`は`Invoke-Expression`のエイリアスです
